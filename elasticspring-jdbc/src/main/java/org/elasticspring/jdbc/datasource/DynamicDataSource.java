...
/**
 * Dynamic data source with regards to the configuration and initialization. This data source is useful if the
 * configuration is not available at configuration time and if the data source initialization is time consuming.
 * <p/>
 * <p><b>Configuration: </b>This data source retrieves all information like host name, port and user through through
 * the {@link DataSourceInformation} object and uses the {@link DataSourceFactory} class to actually instantiate the
 * class. The passed in DataSourceFactory will typically create a data source connection pool through an
...
public final class DynamicDataSource extends AbstractDataSource implements InitializingBean {
	 ...
	 * immutable and therefore can not be changed at runtime. This constructor also initializes the data source. In
	 ...
	 * 		The data source factory implementation that will be used to instantiate the underlying
	 * 		datasource. The data source may contain implementation specific configuration
	 ...
	 * data source is available and already initialized. Both checks are not expensive once the underlying data source has
	 * been initialized.
	 * <p>All the calls to this method blocks until the data source is initialized. In normal situations, the data source
	 * will already be available on the first call of this method. However, the calling code may have to wait until the
	 * data source is initialized or this class is shutdown through the method {@link #destroyDataSource()}.</p>
	 * <p><b>Note:</b>If this instance is shut down while doing a method call, this method will throw an
	 * IllegalStateException</p>
	 *
	 * @return The connection which is provided by the underlying data source. Typically a pooled connection.
	 * @throws SQLException
	 * 		The exception that is thrown by the underlying data source implementation.
	 * @throws IllegalArgumentException
	 * 		If this data source is not active because it has been shutdown while calling this method.
	 */
	@Override
	...
	/**
	 * Destroys the data source and propagates this call to the underlying DataSourceFactory in order to close any open
	 * connections held in a pool. Calling this method also interrupts the initialization process, this might be
	 * especially
	 * the case if the application shutdowns while initializing the data source.
	 */
