...
However when using the topic ARN itself in your application, `SnsTemplate` exposes a method `topicExists` that can be used to validate the SNS topic's existence at application startup itself.
