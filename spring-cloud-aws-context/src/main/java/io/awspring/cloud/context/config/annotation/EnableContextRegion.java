...
/**
 * Configures a {@link RegionProvider} instance for the application context. The region
 * provider will be used for all Amazon Web Service clients that are created inside the
 * application context (by the Spring Cloud AWS classes). A region can be either manually
 * configured {@link EnableContextRegion#region()} with a constant expression, dynamic
 * expression (using a SpEL expression) or a place holder. If the application context is
 * running inside a EC2 instance The region can also be dynamically retrieved from the EC2
