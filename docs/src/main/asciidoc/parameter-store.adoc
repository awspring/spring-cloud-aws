...
|With `optional` application will start even if there is no parameter found for specified parameters.

|`spring.config.import=aws-parameterstore:optional:config-key;other-config-key;optional:some-other-key`
|With `optional` before a key, application will start even if there is no parameter found for specified key. Only key in front of optional will be taken in account.
|===
