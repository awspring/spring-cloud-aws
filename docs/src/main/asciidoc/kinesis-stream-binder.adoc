...
It contains information about its design, usage and configuration options, as well as information on how the Spring Cloud Stream concepts map into AWS Kinesis-specific constructs.
...
The Spring Cloud Stream Binder for AWS Kinesis provides the binding implementation for Spring Cloud Stream.
...
Unlike https://kafka.apache.org/[Apache Kafka], AWS Kinesis doesn't provide out-of-the-box support for consumer groups.
The support of this feature is implemented as part of the `MetadataStore` key for shard checkpoints in the `KinesisMessageDrivenChannelAdapter` - `[CONSUMER_GROUP]:[STREAM]:[SHARD_ID]`.
In addition, the `LockRegistry` is used to ensure exclusive access to each shard.
This ensures that only one channel adapter in the same consumer group will consume messages from a single shard in the stream it is configured for.
...
However, it is not possible to explicitly select a target shard for sending.
By calculating the hash manually and using `explicitHashKeyExpression` on the producer side, the target shard can be tracked by checking inclusion in its `HashKeyRange`.

By default, a partition key is a result of the `Object.hash()` from the message `payload`.

The Spring Cloud Stream partition handling logic is excluded in the case of the AWS Kinesis Binder since it is not used, and the provided `producer.partitionKeyExpression` is propagated directly to the `KinesisMessageHandler`.

On the consumer side, the `instanceCount` and `instanceIndex` are used to evenly distribute shards between consumers in a group.
This has an effect only for a regular `KinesisMessageDrivenChannelAdapter`, which can assign specific shards for the target Kinesis consumer.
...
A single consumer for the message is ensured by the https://docs.spring.io/spring-cloud-stream/reference/spring-cloud-stream/consumer-groups.html[consumer group abstraction].
...
- Use the same group name for the channel in all application instances by using the property `spring.cloud.stream.bindings.<bindingTarget>.group`.
...
But, when that instance goes down (couldn't send a heartbeat for any reason), another instance in the consumer group will start processing from the last checkpoint of the previous consumer (for shardIterator type `TRIM_HORIZON`).
...
It is possible to evenly distribute shards across all instances within a single consumer group.
This is done by configuring:

- `spring.cloud.stream.instanceCount=` to the number of instances
- `spring.cloud.stream.instanceIndex=` to the current instance's index

The only way to achieve HA in this case is that, when an instance processing a particular shard goes down, another instance must have `spring.cloud.stream.instanceIndex=` set to the same value as the failed instance's index in order to start processing from those shards.

== Configuration options

This section contains settings specific to the Kinesis Binder and bound channels.

For general binding configuration options and properties, refer to the https://docs.spring.io/spring-cloud-stream/reference/spring-cloud-stream/configuration-options.html[Spring Cloud Stream core documentation].
...
=== `LockRegistry`
The `LockRegistry` is used to ensure exclusive access to each shard so that only one channel adapter in the same consumer group will consume messages from a single shard in the stream.
...
When `useNativeDecoding = true` is used on the consumer together with the `listenerMode = batch`, there is no out-of-the-box conversion, and the resulting message contains a payload like `List<software.amazon.awssdk.services.kinesis.model.Record>`.
...
If `periodic`, checkpoints occur at a specified time interval (from the `interval` property in checkpoint configuration).
+
Default: `batch`
checkpointInterval::
...
+
No default - if not set, the default value inside spring-integration-aws will be used (random UUID).
fanOut::
...
If `shardIteratorType` is set to `TRIM_HORIZON` explicitly for existing groups, then the stored sequence number is ignored, and the shard iterator is reset to `TRIM_HORIZON`.
+
Default: `LATEST` for anonymous groups and `TRIM_HORIZON` otherwise.

NOTE: When the `TRIM_HORIZON` shard iterator type is used, take into account the time lag that occurs while the `ShardIterator` points to the last untrimmed record in the shard (the oldest data record in the shard).
...
Any subsequent starts of the consumer in the same group are adjusted according to the stored checkpoint via the `AFTER_SEQUENCE_NUMBER` iterator type.

shardId::
An explicit shard id to consume from.

NOTE: Kinesis Client Library does not support a configuration for a specific shard.
When the `shardId` property is used, it is ignored for Kinesis Client Library and standard stream consumer distribution is applied.
Also, in the case of an `instanceCount > 1`, the application will throw a validation exception.
...
Also, if you'd like to produce a batch of records into a Kinesis stream, the message payload must be a `PutRecordsRequest` instance and the general Spring Cloud Stream producer property `useNativeEncoding` must be set to `true`, so Spring Cloud Stream won't try to convert a `PutRecordsRequest` into a `byte[]`. The content of the `PutRecordsRequest` is now the end user's responsibility.
...
When backpressure handling is enabled and the number of records in flight exceeds the threshold, a `KplBackpressureException` would be thrown.
...
If https://docs.aws.amazon.com/streams/latest/dev/what-is-sse.html[Server-Side Encryption] is enabled, you will need the following policies to encrypt and decrypt Kinesis messages.
[source,json]
----
{
    "Version": "2012-10-17",
...
NOTE: The binding destination for the DynamoDB Stream has to be specified as a Stream ARN on the table to capture changes.

[[telling-the-binder-to-use-your-local-endpoint]]
=== Telling the binder to use your local endpoint

By default, the Kinesis and DynamoDB clients will try to hit the real AWS endpoint.
To change this behavior, it is enough to provide these properties for the Spring Cloud AWS auto-configuration:
...
The `LocalstackContainerTest` interface in this project can serve as a good example of how to configure the container and how to use AWS clients.

[[health-indicator]]
== Kinesis binder health indicator

The `KinesisBinderHealthIndicator` implementation is part of the `BindersHealthContributor` composition under the `binders` path.
...
If any of the streams doesn't exist, the health is treated as `DOWN`.
If `LimitExceededException` is thrown according to the `describeStream` limitations, the `KinesisBinderHealthIndicator` tries again after a one-second interval.
Only when all the configured streams are described properly is the `UP` health status returned.
You can override the out-of-the-box implementation by providing your own bean with the `kinesisBinderHealthIndicator` name.
