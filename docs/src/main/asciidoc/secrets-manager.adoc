...
Each block is separated with `#---` and describe one document. Only block that `spring.config.activate.on-profile` is equal to active profile in Application Context are used.
