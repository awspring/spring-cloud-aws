...
public final class QueueMessageUtils {
	...
	public static Message<String> createMessage(software.amazon.awssdk.services.sqs.model.Message message) {
	...
	public static Message<String> createMessage(software.amazon.awssdk.services.sqs.model.Message message,
		...
		messageHeaders.put(RECEIPT_HANDLE_MESSAGE_ATTRIBUTE_NAME, message.receiptHandle());
	...
	public static software.amazon.awssdk.services.sqs.model.Message getSourceData(Message<?> message) {
		return (software.amazon.awssdk.services.sqs.model.Message) message.getHeaders().get(SOURCE_DATA_HEADER);
	}

	private static Map<String, Object> getAttributesAsMessageHeaders(software.amazon.awssdk.services.sqs.model.Message message) {
		return new HashMap<>(message.attributesAsStrings());
	}
	...
	private static Map<String, Object> getMessageAttributesAsMessageHeaders(
		...
		for (Map.Entry<String, MessageAttributeValue> messageAttribute : message.messageAttributes().entrySet()) {
			if (MessageHeaders.CONTENT_TYPE.equals(messageAttribute.getKey())) {
				...
				messageHeaders.put(MessageHeaders.ID, UUID.fromString(messageAttribute.getValue().stringValue()));
			}
			else if (messageAttribute.getValue().dataType().startsWith(MessageAttributeDataTypes.STRING)) {
				messageHeaders.put(messageAttribute.getKey(), messageAttribute.getValue().stringValue());
			}
			else if (messageAttribute.getValue().dataType().startsWith(MessageAttributeDataTypes.NUMBER)) {
				messageHeaders.put(messageAttribute.getKey(), getNumberValue(messageAttribute.getValue()));
			}
			else if (messageAttribute.getValue().dataType().startsWith(MessageAttributeDataTypes.BINARY)) {
				messageHeaders.put(messageAttribute.getKey(), messageAttribute.getValue().binaryValue());
			}
		}
