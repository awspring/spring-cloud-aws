...
The same behavior can be enabled using Spring Boot integration with Spring Security OAuth2 support:
...
The example above sets the URIs to LocalStack's Cognito server.
...
For example, if all you had done was set `aws.paramstore.enabled=true`, and your `spring.application.name` was `my-service`, then parameter store items could be resolved from either `/config/application` (for shared config items) or `/config/my-service` (for application-specific config items).
