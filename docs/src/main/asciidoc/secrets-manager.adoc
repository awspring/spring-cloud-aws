...
|`spring.config.import=aws-secretsmanager:optional:secret-key;other-secret-key;optional:some-other-key`
|With `optional` before a key, application will start even if there is no secret found for specified key. Only key in front of optional will be taken in account.
|===
