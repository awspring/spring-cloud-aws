...
For standard queues, all batches are sent in parallel. For FIFO queues, we send the batches in parallel grouped by message group ID.
...
Send failures within a batch can surface in the following ways:
