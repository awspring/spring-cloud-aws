...
|cloud.aws.sqs.listener.fail-on-missing-queue | `false` | Decides if application should fail on startup when {@link SqsListener} is used for a Queue that does not exist.
