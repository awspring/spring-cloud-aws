...
== SES Integration
Spring provides built-in support for sending e-mails through the https://www.oracle.com/technetwork/java/javamail/index.html[Java Mail API],
...
the standard Java Mail API-based implementation that sends e-mails via SMTP to Amazon SES.
...
Application developers can inject the `MailSender` into their application code to send simple text-based e-mail
...
Sending e-mail attachments or HTML e-mails requires MIME messages. To create MIME messages,
