...
----
 <beans ...>
  ...
  <jdbc:data-source ... region="EU_WEST_1" />
 </beans>
----
...
examples shows the implementation of on configurer that overrides the validation query and the initial size.
...
TIP: This class returns an anonymous class of type `org.springframework.cloud.aws.jdbc.config.annotation.RdsInstanceConfigurer`,
 that class might be of course also a standalone class.
...
on the `data-source` element. Configuring the tags support like in the example below:
