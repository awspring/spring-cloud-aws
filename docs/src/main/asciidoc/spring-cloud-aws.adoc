...
against the AWS cloud). Each integration can now be configured to have a specific region. If region is specified this way it takes priority over default or static region.
...
Each integration can be enable or disable autoconfiguration. By default all integrations are enabled beside ContextInstanceDataAutoConfiguration and CloudFormation.

The properties to configure the enabling are shown below
...
|false
|This disables SQS integration autoconfiguration.
|===
