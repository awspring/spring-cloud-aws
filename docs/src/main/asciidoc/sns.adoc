...
Auto-configuration will then automatically configure `SnsTemplate` with that resolver.
...
When using a topic ARN directly, `SnsTemplate` provides a `topicExists` method to validate that the SNS topic exists at application startup.
...
In such cases, we recommend using `SnsOperations` — an interface implemented by `SnsTemplate` — which exposes a convenient API for sending SNS notifications, including support for FIFO topics.
...
Example of integration (works identically for both Spring MVC and Spring WebFlux):
