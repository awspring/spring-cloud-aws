...
caching abstraction providing a cache manager based on the memcached and the redis protocol. The caching support for Spring
Cloud AWS provides an own memcached implementation for Elasitcache and uses
http://projects.spring.io/spring-data-redis/[Spring Data Redis] for Redis caches.

=== Configuring dependencies for Redis caches
Spring Cloud AWS delivers an own implementation of a memcached cache, therefore no other dependencies are needed. For redis
