...
* use th system properties `aws.accessKeyId` and `aws.secretKey`
...
Following the Spring Cloud umbrella project, Spring Cloud AWS provides also dedicated Spring Boot support. Spring Cloud
...
Additional dependencies to enable particular features like messaging and jdbc have to been added. Spring Cloud AWS will
only configure classes that are available in the Spring Boot applications classpath.
...
===== Configuring region
Like for the credentials, the Spring Cloud AWS module supports also the configuration of the region inside the Spring
...
|EU_WEST_1
|Configures a static region for the application. Possible regions are (currently) US_EAST_1, US_WEST_1, US_WEST_2,
EU_WEST_1, EU_CENTRAL_1, AP_SOUTHEAST_1, AP_SOUTHEAST_2, AP_NORTHEAST_1, SA_EAST_1, CN_NORTH_1
|===
...
The instance meta-data can also be configured within a Java configuration class without the need for an XML configuration.
...
The instance meta-data is automatically available in a Spring Boot application as a property source if the application
is running on a EC2 instance.
...
[NOTE]
====
User data stored in a base 64 encoded representation is automatically decoded by Spring Cloud AWS
====
...
A CloudFormation template for the configuration of the instance tags is outlined below:
...
the application context, they can only be referenced as expressions and not with placeholders. The `context-instance-placeholder`
...
`context-instance-placeholder` element supports a custom EC2 client with the `amazon-ec2` attribute. The next
...
the particular stack. The `dataSource` element uses the logical name for the `db-instance-identifier` attribute to work with
the database.
...
<beans xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <jdbc:dataSource id="dataSource" db-instance-identifier="applicationDatabase" ... />
</beans>
----
...
Spring Cloud AWS also supports the configuration of the CloudFormation support within Java classes avoiding the use of
 ...
 explicit stack name (here `anualStackName`).
...
[TIP]
====
Do not define the `tackName` attribute if an automatic stack name should be enabled.
====

=== CloudFormation configuration in Spring Boot
Spring Cloud AWS support also the configuration of the CloudFormation support within the Spring Boot configuration. The
...
into his maven configuration. Spring Cloud AWS Messaging support comes as a separate module to allow the modularized use
of the modules.
...
with queues. Compared to JMS or other message services Amazon SQS has several features and limitation that should be
...
* Amazon SQS has no transaction support, messages might therefore be retrieved twice. Application have to be written in
...
SQS based subscriptions can be used with the annotation-driven message support that is provided by the Spring Cloud AWS messaging.
...
configure the SNS client as setup the default converter.
...
== Caching
Caching in a cloud environment is useful for applications in the cloud to reduce the latency and to save database round trips.
...
is already imported in the project. The cache integration provides an own namespace to configure cache clusters that are
...
The `cache` and `cache-cluster` elements accept an expiration attribute that defines the expiration time in seconds.
...
annotation provided by Spring Cloud AWS. The next example shows a configuration of two cache clusters using the `lusters`
attribute.
...
IMPORTANT: If you leave the `clusters` attribute empty, then all the caches inside your CloudFormation stack (if available)
 ...
 * Automatic datasource configuration and setup based on the Amazon RDS database instance.
...
The datasource configuration requires the security and region configuration as a minimum allowing Spring Cloud AWS to retrieve
...
to configure the datasource with the minimum attributes as shown in the example:
...
The minimum configuration parameters are a unique `id` for the datasource, a valid `db-instance-identifier` attribute
...
http://tomcat.apache.org/tomcat-7.0-doc/jdbc-pool.html[Tomcat JDBC pool] with the default properties. The datasource
...
It is possible to qualify the datasource injection point with an `@Qualifier` annotation to allow multiple datasource
...
requirements of the application with regards to pool size and other settings. The datasource configuration supports the configuration
of all valid pool properties with a nested XML element. The following example demonstrates the re-configuration of the datasource
...
`rg.springframework.cloud.aws.jdbc.config.annotation.EnableRdsInstance` annotation can be used to configure one data
...
TIP: The configuration attributes are the same like in the XML element. The required attributes are also the same like
...
|cloud.aws.rds.test
|
|The configuration property the configures a data source with the name test
...
[NOTE]
====
In most cases it is better to provide a user direct feedback instead of trying potentially long and often retries within
...
Spring Cloud AWS supports database instance that are configured with CloudFormation. Spring Cloud AWS can use the logical
...
Allows the developer to access the properties in the code using expressions like shown in the class below:
...
use the standard Java Mail API based implementation that send e-mails via SMTP to Amazon SES
...
an SES service, the client is overridden to us a valid region EU-WEST-1.
...
to load and write resources with the resource loader and fa `s3` protocol.
...
Spring Cloud AWS does not modify the default resource loader until an explicit configuration with an XML namespace element.
...
interface which is a specialization of the `rg.springframework.core.io.ResourceLoader` interface. Clients can upload files
with the use of the `WritableResource` interface. The next example demonstrates an upload of a resource using the resource loader.
...
a memory footprint of 5 MB in the heap, a thread size of 10 will consume therefore up to 50 mb of heap space. Spring Cloud
