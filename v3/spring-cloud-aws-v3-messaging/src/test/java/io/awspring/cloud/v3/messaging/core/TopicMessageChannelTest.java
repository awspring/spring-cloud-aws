...
class TopicMessageChannelTest {
	...
	void sendMessage_validTextMessageAndSubject_returnsTrue() throws Exception {
		...
		// Assert
		verify(amazonSns, only()).publish(eq(PublishRequest.builder()
			.topicArn("topicArn")
	...
	void sendMessage_withStringMessageHeader_shouldBeSentAsTopicMessageAttribute() throws Exception {
		...
		when(amazonSns.publish(publishRequestArgumentCaptor.capture())).thenReturn(PublishResponse.builder().build());
	...
	void sendMessage_withUuidAsId_shouldConvertUuidToString() throws Exception {
		...
		when(amazonSns.publish(sendMessageRequestArgumentCaptor.capture())).thenReturn(PublishResponse.builder().build());
