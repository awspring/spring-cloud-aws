...
caching abstraction providing a cache manager based on the memcached and the redis protocol. The caching support fo Spring
Cloud AWS provides an own implementation memcached implementation for Elasitcache and uses
http://projects.spring.io/spring-data-redis/[Spring Data Redis] for Redis caches.

=== Configuring dependencies for Redis caches
Spring Cloud AWS delivery an own implementation of a memcached cache, therefore no other dependencies are needed. For memcached
...
caches using the `defaultExpiration` attribute as show in the example below.
...
or on a cache level using the `@CacheClusterConfig` annotations expiration attribute as shown below.
