...
import io.awspring.cloud.v3.messaging.listener.support.AcknowledgmentHandlerMethodArgumentResolver;
import io.awspring.cloud.v3.messaging.listener.support.VisibilityHandlerMethodArgumentResolver;
import io.awspring.cloud.v3.messaging.support.NotificationMessageArgumentResolver;
import io.awspring.cloud.v3.messaging.support.NotificationSubjectArgumentResolver;
import io.awspring.cloud.v3.messaging.support.SqsHeadersMethodArgumentResolver;
...
public class QueueMessageHandler extends AbstractMethodMessageHandler<QueueMessageHandler.MappingInformation> {
	...
	protected MappingInformation getMappingForMethod(Method method, Class<?> handlerType) {
		...
		if (sqsListenerAnnotation != null && sqsListenerAnnotation.value().length > 0) {
			SqsMessageDeletionPolicy tempDeletionPolicy =
				Objects.equals(sqsListenerAnnotation.deletionPolicy(), SqsMessageDeletionPolicy.DEFAULT)
