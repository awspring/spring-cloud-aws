...
In EKS, follow this guide to set up service accounts: https://docs.aws.amazon.com/eks/latest/userguide/pod-configuration.html[EKS Pod Configuration].
...
STS credentials configuration supports the following properties:
...
There are two types of AWS clients - synchronous and asynchronous. Each Spring Cloud AWS integration uses one or the other type:
...
Client-specific customizations can be applied through client-specific customizer interfaces (for example `S3ClientCustomizer` for S3). See the integrations documentation for details.


=== GraalVM Native Image

Since version 3.3.0, the framework provides **experimental** support for building a GraalVM Native Image.
...
- In S3 integration, when working with the CRT client, the following guide must be followed: https://github.com/awslabs/aws-crt-java?tab=readme-ov-file#graalvm-support
