...
import io.awspring.cloud.v3.core.naming.AmazonResourceName;
...
public class DynamicTopicDestinationResolver implements DestinationResolver<String> {
	...
	public DynamicTopicDestinationResolver(SnsClient amazonSns, ResourceIdResolver resourceIdResolver) {
	...
	public String resolveDestination(String name) throws DestinationResolutionException {
		if (this.autoCreate) {
			return this.amazonSns.createTopic(CreateTopicRequest.builder()
					.name(name)
				.build()).topicArn();
		}
		else {
	...
	private String getTopicResourceName(String marker, String topicName) {
		ListTopicsResponse listTopicsResponse = this.amazonSns.listTopics(ListTopicsRequest.builder()
			.nextToken(marker)
			.build());
		...
		if (StringUtils.hasText(listTopicsResponse.nextToken())) {
			return getTopicResourceName(listTopicsResponse.nextToken(), topicName);
		}
		else {
