...
For standard queues, all batches are sent in parallel. For FIFO queues, batches are sent sequentially to preserve the message order, with messages grouped by message group ID.
