...
import static io.awspring.cloud.autoconfigure.cloudmap.CloudMapUtils.ECS;
import static io.awspring.cloud.autoconfigure.cloudmap.CloudMapUtils.ECS_CONTAINER_METADATA_URI_V_4;

/**
 * Integration test for Ecs based Cloud Map registration.
 *
...
public class EcsCloudMapIntegrationTest {
	 ...
	 */
	@Test
	void registerEcsContainerWithCloudMapWithNoSpecification() {
		SpringApplication application = new SpringApplication(EcsApp.class);
		...
		properties.add("--spring.cloud.aws.cloudmap.deploymentPlatform=" + ECS);
	 ...
	 */
	@Test
	void registerEcsContainerWithCloudMapWithDefinedSpecification() {
		SpringApplication application = new SpringApplication(EcsApp.class);
		application.setWebApplicationType(WebApplicationType.NONE);
		application.addBootstrapRegistryInitializer(new AwsConfigurerClientConfiguration());
	 ...
	 */
	@Test
	void discoverEcsCloudMapInstancesWithNoSpecification() {
	...
	static class EcsApp {
		static {
			System.setProperty(ECS_CONTAINER_METADATA_URI_V_4, IntegrationTestUtil.META_DATA_MOCK_RESPONSE);
		}
	}
