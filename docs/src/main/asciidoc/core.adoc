...
=== Custom SDKClient and ClientOverrideConfiguration

To configure Client with custom SDKClient and ClientOverrideConfiguration simply implement `AwsClientConfigurer` interface and register class as a Bean.
Autoconfiguration module will automatically pick up the Bean of type `AwsClientConfigurer` and configure Client with given properties.
In our case this will be S3Client, since we specified S3ClientBuilder as generic type, example:
