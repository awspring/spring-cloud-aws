...
Also, you will need to enable STS AutoConfiguration by specifying `spring.cloud.aws.credentials.sts.enabled:true`.
...
1. Defining inside of `application.properties` - `spring.cloud.aws.credentials.sts.roleArn` and `spring.cloud.aws.credentials.sts.webIdentityTokenFile`
2. Java System Properties - `aws.roleArn` and `aws.webIdentityTokenFile`
3. Environment Variables - `AWS_ROLE_ARN` and `AWS_WEB_IDENTITY_TOKEN_FILE`
...
1. Defining inside of `application.properties` - `spring.cloud.aws.credentials.sts.roleSessionName` and `spring.cloud.aws.credentials.sts.isAsyncCredentialsUpdate`
2. Java System Properties - `aws.roleSessionName` and `aws.isAsyncCredentialsUpdate`
3. Environment Variables - `AWS_ROLE_SESSION_NAME` and `AWS_IS_ASYNC_CREDENTIALS_UPDATE`
