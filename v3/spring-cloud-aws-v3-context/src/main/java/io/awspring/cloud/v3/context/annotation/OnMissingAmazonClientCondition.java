...
package io.awspring.cloud.v3.context.annotation;
...
public class OnMissingAmazonClientCondition implements Condition {
	...
	private boolean isAmazonClientMissing(ConditionContext context, String amazonClientClass) {
		return !context.getBeanFactory().containsBean(amazonClientClass)
				&& !context.getRegistry().containsBeanDefinition(amazonClientClass);
	}
