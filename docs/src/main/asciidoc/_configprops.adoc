...
|spring.cloud.aws.s3.vector.dualstack-enabled |  | Configure whether the AWS client should use the AWS dualstack endpoint. Note that not each AWS service supports dual-stack. For complete list check <a href="https://docs.aws.amazon.com/vpc/latest/userguide/aws-ipv6-support.html">AWS services that support IPv6</a>
|spring.cloud.aws.s3.vector.endpoint |  | Overrides the default endpoint.
|spring.cloud.aws.s3.vector.region |  | Overrides the default region.
...
|spring.cloud.aws.sqs.observation-enabled | `+++false+++` | 
