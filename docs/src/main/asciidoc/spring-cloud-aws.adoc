...
==== Enabling instance metadata support
...
on instance startup. Therefore it is useful to leverage instance tags for user configuration and user data to script
...
Spring Cloud AWS ships with the Amazon ElastiCache client as an inlined dependency to ensure that there is no
