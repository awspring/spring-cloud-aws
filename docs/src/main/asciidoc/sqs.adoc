...
- `messageGrouping` - Specifies whether messages from FIFO queues must be grouped by message group when listened to in batch mode. See <<FIFO Support>> for more information.
...
* If a `batch` listener is used, each message group from a poll will be served as a batch to the listener method. To receive messages from different groups in a `batch`, `message grouping` property must be set to false through `@SqsListener` or `SqsContainerOptions`.
