...
must match the queue name defined on AWS. This value will be resolved at runtime with the Amazon SQS client. Optionally
a `ResourceIdResolver` implementation can be passed to the `QueueMessagingTemplate` constructor to resolve resources by
logical name when running inside a CloudFormation stack.
...
<<Managing cloud environments>> for more information about resource resolution).
