...
== Messaging
Spring Cloud AWS provides a SQS and SNS integration framework that simplifies the publication and consumption of messages
over SQS or SNS. While SQS fully relies on the messaging API introduced with Spring 4.0, SNS only partially implements
it as the receiving part must be handled differently for push notifications.
...
=== SNS support
Spring Cloud AWS supports SNS by providing a `NotificationMessagingTemplate` for sending notifications and annotation
support to map HTTP notification endpoints.
...
http://docs.aws.amazon.com/ses/latest/DeveloperGuide/request-production-access.html[Production access] can be requested
 ...
 == File handling (S3)
 Spring Cloud AWS provides support for writing and reading files stored on S3.