...
class SqsConfigurationTest {
	...
	void configuration_withMinimalBeans_shouldStartSqsListenerContainer() throws Exception {
		...
		SqsClient amazonBufferedSqsClient = (SqsClient) ReflectionTestUtils
				.getField(messagingTemplate, "amazonSqs");
		SqsClient amazonSqsClient = (SqsClient) ReflectionTestUtils
	...
	void configuration_withoutAwsCredentials_shouldCreateAClientWithDefaultCredentialsProvider() throws Exception {
		...
		// Assert
		SqsClient bufferedAmazonSqsClient = applicationContext
				.getBean(SqsClient.class);
		SqsClient amazonSqsClient = (SqsClient) ReflectionTestUtils
	...
	void configuration_withRegionProvider_shouldUseItForClient() throws Exception {
		...
		SqsClient bufferedAmazonSqsClient = applicationContext.getBean(SqsClient.class);
		SqsClient amazonSqs = (SqsClient) ReflectionTestUtils.getField(bufferedAmazonSqsClient,
				"realSQS");
	...
	static class ConfigurationWithCustomAmazonClient {
		...
		static final SqsClient CUSTOM_SQS_CLIENT = mock(SqsClient.class, withSettings().stubOnly());
	...
	static class ConfigurationWithCustomizedMessageHandler extends MinimalConfiguration {
		...
		static final SqsClient CUSTOM_AMAZON_SQS = mock(SqsClient.class, withSettings().stubOnly());
	...
	static class ConfigurationWithCustomContainerFactory {
		...
		static final SqsClient AMAZON_SQS = mock(SqsClient.class, withSettings().stubOnly());
