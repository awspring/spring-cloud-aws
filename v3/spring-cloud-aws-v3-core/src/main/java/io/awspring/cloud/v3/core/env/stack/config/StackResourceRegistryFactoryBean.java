 ...
 * {@link StackResourceRegistry} instance representing
...
public class StackResourceRegistryFactoryBean extends AbstractFactoryBean<ListableStackResourceFactory> {
	...
	public StackResourceRegistryFactoryBean(CloudFormationClient amazonCloudFormationClient,
	...
	private List<StackResourceSummary> getStackResourceSummaries(String stackName) {
		ListStackResourcesResponse listStackResourcesResult = this.amazonCloudFormationClient
				.listStackResources(ListStackResourcesRequest.builder().stackName(stackName).build());
		if (!StringUtils.hasText(listStackResourcesResult.nextToken())) {
			return listStackResourcesResult.stackResourceSummaries();
		}

		else {
			List<StackResourceSummary> result = new ArrayList<>(listStackResourcesResult.stackResourceSummaries());
				...
				listStackResourcesResult = this.amazonCloudFormationClient
						.listStackResources(
							ListStackResourcesRequest.builder()
								.stackName(stackName)
								.nextToken(listStackResourcesResult.nextToken())
								.build());
	...
	private Map<String, StackResource> convertToStackResourceMappings(String prefix,
		...
		for (StackResourceSummary stackResourceSummary : stackResourceSummaries) {
			String logicalResourceId = toNestedResourceId(prefix, stackResourceSummary.logicalResourceId());
			stackResourceMappings.put(logicalResourceId, new StackResource(logicalResourceId,
					stackResourceSummary.physicalResourceId(), stackResourceSummary.resourceType()));
		}
