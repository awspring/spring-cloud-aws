...
instance meta-data inside an XML file using placeholder and also the expression referring to the special variable `environment`
...
Instance meta-data can also be injected with the Spring `org.springframework.beans.factory.annotation.Value` annotation
...
User configured properties can also be configured with tags instead of user data. Tags are a global concept in the context
 of Amazon Web services and used in different services. Spring Cloud AWS support instance tags also across different
...
and security configuration before actually resolving the placeholder. Also because the instance tags are not available while starting
 the application context, they can be only referenced as expression and not with placeholders. The `context-instance-placeholder`
...
A java bean might resolve expression with the `@Value` annotation.
...
`context-instance-placeholder` element support custom EC2 client with the `amazon-ec2` attribute. The next
...
By default Spring Cloud AWS configures an EC2 client with the specified region and security credentials. Application
 developers can inject the EC2 client directly into their code using the `@Autowired` annotation.
...
The datasource is then created and will receive a physical name (e.g. ir142c39k6o5irj) as the database service name. Application
 developers can still use the logical name (in this case `applicationDatabase`) to interact with the database. The example shows
 the stack configuration which is defined by the element `aws-context:stack-configuration` and resolves automatically the
 particular stack. The `dataSource` element uses the logical name for the `db-instance-identifier` attribute to work with
 the database.
...
manually. The configuration consistent of an additional element `stack-name` attribute that will be used to resolve all the
...
The application can then access the stack tags with a expression like `#{stackTags.key1}`
...
client with a special setup. The client itself can be configured using the `amazon-cloud-formation` attribute
...
== Messaging
Spring Cloud AWS provides a http://aws.amazon.com/sqs/[Amazon SQS] and http://aws.amazon.com/sqs/[Amazon SNS] integration
...
into his maven configuration. Spring Cloud AWS Messaging support comes as a separate module to allow the modularized use of the
modules.
...
with queues. Compared to JMS or other message service Amazon SQS has several features and limitation that should be
taken into consideration

* Amazon SQS allows only String payloads, any Object must be transformed into a String representation.
	Spring Cloud AWS  has dedicated support to transfer Java object with Amazon SQS messages by converting them to JSON.
* Amazon SQS has no transaction support, message might therefore be retrieved twice. Application have to be written in
	a idempotent way so that they can receive a message twice.
* Amazon SQS has a maximum message size of 256kb per message, bigger messages will fail to be sent.
...
NOTE: As SQS is only able to send `String` payloads the default converter `SimpleMessageConverter` should only be used
...
annotation. The incoming messages are converted to the target type and then annotated method `queueListener` is invoked.

Additionally to the payload, headers can be injected in the listener methods with the `@Header` or `@Headers`
...
NOTE: Only the link:http://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/API_Message.html[standard
...
`aws-messaging:annotation-driven-queue-listener` element using the `aws-messaging:argument-resolvers` (see example below).
...
max pools size. The core pool size is set to twice the number of queues and the max pool size is obtained by multiplying
...
[source,xml,indent=0]
----
<aws-messaging:annotation-driven-queue-listener annotation-driven-queue-listener="queueMessagingTemplate"/>
----
...
`aws-messaging:annotation-driven-queue-listener` XML element there is an attribute `annotation-driven-queue-listener`
that specifies `QueueMessagingTemplate` as the messaging template to be used to send the return value of the message
listener method.

=== SNS support
Amazon SNS is a publish-subscribe messaging system that allows client to publish notification to a particular topic. Other
interested clients may subscribe using different protocols like HTTP/HTTPS, E-Mail or a Amazon SQS queue ti receive the messages.

The next graphic shows a typical example of an Amazon SNS architecture.

image::sns-overview.png[SNS Overview]

Spring Cloud AWS supports Amazon SNS by providing support to send notification with a NotificationMessagingTemplate` and
to receive notifications with the HTTP/HTTPS endpoint using the Spring Web MVC '@Controller' based programming model. Amazon
 SQS based subscription can be used with annotation driven message that is provided by the Spring Cloud AWS messaging support.
...
NOTE: Currently only `String` payloads can be sent using the `NotificationMessagingTemplate` as this is the expected
type by the SNS API.
...
CAUTION: Currently it is not possible to define the mapping URL on the method level therefore the `RequestMapping` must
...
== Caching
Caching in a cloud environment is useful for application in the cloud to reduce the latency and to save database round trips.
Reducing database round trips can significantly reduce the requirements for the database instance. The Spring Framework
provides since version 3.1 a unified Cache abstraction to allow declarative caching in application analogous to the
declarative transactions.

Spring Cloud AWS integrates the http://aws.amazon.com/elasticache/[Amazon ElastiCache] service into the Spring unified
caching abstraction providing a cache manager based on the memcached protocol.

[TIP]
====
Support for the redis cache protocol is planned in a future release.
====

=== Configuring Caching
The cache support for Spring Cloud AWS resides in the context module and can therefore be used if the context module
is already imported in the project. The cache integration provides an own namespace to configure cache cluster that are
...
=== Using caching
Based on the configuration of the cache, developer scan annotate their methods to use the caching for method return values.
The next examples contains a caching declaration for a service for which the return values should be cached
...
The example below demonstrates a configuration example that contains a pre-configured cache with an `cache-ref` element
(which might be a local cache),a `cache` with an explicit address (that is probably hosted outside Elasticache) and
...
<beans ...>
	<aws-cache:cache-manager id="cacheManager">
		...
		<aws-cache:cache address="someLocalServer:4321" name="memc" />
...
expiry time will be passed to the memcache service.

The `cache` and `cache-cluster` elements accept a expiration attribute that defines the expiration time in seconds.
...
<beans>
	<aws-cache:cache-manager>
		<aws-cache:cache address="someLocalServer:4321" name="memc" expiration="423"/>
...
Spring Cloud AWS ships with the Amazon ElastiCache client as an in-lined dependency to ensure that there is no
wrong cache client implementation (e.g. a stock spymemcached) in the classpath that does not support the
dynamic endpoint based re-configuration.

=== Using CloudFormation
Amazon ElastiCache Cluster can also be configured within a stack and then be used by applications. Spring Cloud AWS
also support the lookup of stack-configured cache clusters by their logical name with the resolution to the physical
...
With the configuration above the application can be deployed with multiple stack on different environment
...
the database meta-data information with the Amazon RDS service. Spring Cloud AWS provides an additional `jdbc` specific namespace
...
The minimum configuration parameters are an unique `id` for the datasource, a valid `db-instance-identifier` attribute
...
The read-replica support can be enabled with the `read-replica` attribute in the data source configuration.
...
configuration meta-data with the database. Database instance specific can be configured using the `user-tags-map` attribute
...
Spring Cloud AWS provides a XML element to configure a Spring `org.springframework.mail.MailSender` implementation for the
...
Sending the mail requires the application developer to use the `JavaMailSender` to send an E-Mail that is shown in the example
below.
...
e-mail receiver otherwise the mail sender will throw an `com.amazonaws.services.simpleemail.model.MessageRejectedException`.
...
== Resource handling
The Spring Framework provides a org.springframework.core.io.ResourceLoader abstraction to load files from the file system,
servlet context and the class path. Spring Cloud AWS add support for the http://aws.amazon.com/s3/[Amazon S3] service
to load and write resource with the resource load and a `s3`protocol.
...
Spring Cloud AWS does nit modify the default resource loader until an explicit configuration with a XML namespace element.
 The configuration consist one element for the whole application context that is shown below
...
The next example demonstrates the use of the resource load to load different resources.
...
=== Uploading files
Since Spring Framework 3.1 the resource load can also be used to upload file with the org.springframework.core.io.WritableResource`
which is a specialization of the `rg.springframework.core.io.ResourceLoader` interface. Clients can upload file with the use
of the `WitableResource` interface. The next example demonstrates an upload of a resource using the resource loader.
...
Amazon S3 supports the upload with http://docs.aws.amazon.com/AmazonS3/latest/dev/uploadobjusingmpu.html[multi parts] to
increase the general throughput while uploading. Spring Cloud AWS by default only uses one thread to upload the files and
therefore does not provide parallel upload support. Users can configure a custom `or.springframework.core.task.TaskExecutor`
...
[WARNING]
====
Spring Cloud AWS consumes up to 5 MB (the minimum part size) of memory per thread. Therefore each parallel thread will incur
a memory footprint of 5 MB in the heap, a thread size fo 10 will consume therefore up to 50 mb of heap space. Spring Cloud
AWS releases the memory ass soon as possible. Also the example above shows that there is no `ueue-capacity` configured,
because queued request would also consume memory.
====
...
the multi-part functionality. A `com.amazonaws.services.s3.transfer.TransferManager` can be easily created in the application
 code and injected with the already pre-configured com.amazonaws.services.s3.AmazonS3` client that is already created
 with the Spring Cloud AWS resource loader configuration.

The examples show the use of the `transferManager` within an application to upload files from the hard-drive.
...
=== Searching resources
The Spring resource loader also supports collecting resources based on a ANT-style path specification. Spring Cloud AWS
 ...
 resolution by using different patterns.
...
[WARNING]
====
Resolving resources throughout all buckets can be very time consuming based on the number of buckets a user owns.
====

=== Using CloudFormation
CloudFormation also allows to create buckets during stack creation. These bucket will typically have a generated name
...
Application developers can use the `org.springframework.cloud.aws.core.env.ResourceIdResolver` interface to resolve the
 physical name that are generated based on the logical names.
