...
|`PROCESS_MESSAGE_GROUPS_IN_PARALLEL_BATCHES`, `PROCESS_ALL_GROUPS_IN_EACH_BATCH`
...
* If a `batch` listener is used, each message group from a poll will be served as a batch to the listener method. To receive messages from different groups in a `batch`, `batchGroupingStrategy` property must be configured in `SqsContainerOptions`.
