...
* If a `batch` listener is used, each message group from a poll will be served as a batch to the listener method. To receive messages from multiple groups in a `batch`, `fifoBatchGroupingStrategy` property must be configured in `SqsContainerOptions`.
