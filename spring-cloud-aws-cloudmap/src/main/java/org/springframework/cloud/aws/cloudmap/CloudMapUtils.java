...
public enum CloudMapUtils {
	...
	final Logger LOGGER = LoggerFactory.getLogger(CloudMapUtils.class);
	final ObjectMapper JSON_MAPPER = new ObjectMapper();
	final String VPC_ID = "VPC_ID";
	final String AWS_INSTANCE_IPV_4 = "AWS_INSTANCE_IPV4";
	...
	public Map<String, String> getRegistrationAttributes() {
		...
		try {
			...
			attributes.put(IPV_4_ADDRESS, generateIP());
			attributes.put(VPC_ID, "vpc-13c33d6e");
		}
			...
			discoveryProperties.forEach(d -> {
				...
				do {
					...
					if (StringUtils.hasText(nameSpaceId)) {
						...
						if (token != null)
							servicesRequest.withNextToken(token);
						...
						if (StringUtils.hasText(serviceName)) {
							serviceList.addAll(result.getServices().stream()
								.filter(r -> r.getName().equalsIgnoreCase(d.getService()))
								.map(r -> generateServiceId(r.getName(), nameSpace))
								.collect(Collectors.toList()));
							doContinue = (serviceList.size() != discoveryProperties.size());
						} else
							serviceList.addAll(result.getServices().stream()
								.map(r -> generateServiceId(r.getName(), nameSpace))
								.collect(Collectors.toList()));
					}
				} while (doContinue && StringUtils.hasText(token));
			});
		}
	 ...
	 */
	public List<HttpInstanceSummary> listInstances(final AWSServiceDiscovery serviceDiscovery,
												   final String namespace, String serviceName) {
		final DiscoverInstancesRequest dRequest = new DiscoverInstancesRequest()
	 ...
	 */
	private String createNameSpace(AWSServiceDiscovery serviceDiscovery, CloudMapRegistryProperties properties, String vpcId)
