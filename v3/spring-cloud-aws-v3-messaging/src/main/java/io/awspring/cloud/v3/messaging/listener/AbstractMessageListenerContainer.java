...
abstract class AbstractMessageListenerContainer
	...
	private QueueAttributes queueAttributes(String queue, SqsMessageDeletionPolicy deletionPolicy) {
		...
		GetQueueAttributesResponse queueAttributes = getAmazonSqs()
		...
		boolean hasRedrivePolicy = queueAttributes.attributes().containsKey(QueueAttributeName.REDRIVE_POLICY);
	...
	protected static class QueueAttributes {
		...
		public ReceiveMessageRequest getReceiveMessageRequest() {
			ReceiveMessageRequest.Builder builder = ReceiveMessageRequest
				.builder()
				.queueUrl(this.destinationUrl)
				.attributeNames(QueueAttributeName.ALL)
				.messageAttributeNames(RECEIVING_MESSAGE_ATTRIBUTES)
				.maxNumberOfMessages(Optional
					.ofNullable(this.maxNumberOfMessages).orElse(DEFAULT_MAX_NUMBER_OF_MESSAGES));

			if (this.visibilityTimeout != null) {
				builder = builder.visibilityTimeout(this.visibilityTimeout);
			}

			if (this.waitTimeOut != null) {
				builder = builder.waitTimeSeconds(this.waitTimeOut);
			}
